...
class ThumbnailMakerService(object):
    def __init__(self, home_dir='.'): 
        ...
        self.dl_lock = threading.Lock() #lock used to synchronize threads
        max_concurrent_dl = 4
    ...
    def download_image(self, url):
        ...
        with self.dl_sem:
            ...
            #Critical Section
            with self.dl_lock:
                self.downloaded_bytes += img_size # downloaded_bytes is shared variable and could be lost between thread calls
    ...
    def download_images(self, img_url_list):
        ...
        threads = [] # Create a thread list to keep track of all threads
        for url in img_url_list:
            t = threading.Thread(target=self.download_image, args=(url,))
            t.start()
            threads.append(t) # Add thread to list

        # Loop through all threads and make the main thread wait for all other threads to finish
        for t in threads:
            t.join()
